...
-- Movimientos de inventario, proformas, requerimientos y órdenes de compra.
-- Usa subqueries por nombre/código para evitar hardcodear IDs.
...
BEGIN
  ...
  IF (SELECT COUNT(*) FROM movimientos_inventario) = 0 THEN
    ...
    -- Ingresos con factura (hace 60-45 días)
    ...
    UPDATE materiales SET stock_actual =    5, updated_at = NOW() WHERE codigo = 'MAT-003'; -- crítico (mín 10)
    ...
    UPDATE materiales SET stock_actual =   15, updated_at = NOW() WHERE codigo = 'MAT-009'; -- bajo mínimo (10)
    UPDATE materiales SET stock_actual =  600, updated_at = NOW() WHERE codigo = 'MAT-010';

    -- Alertas de stock generadas automáticamente
  ...
  -- ── PROFORMAS + PARTIDAS + REQUERIMIENTOS ────────────────────
  IF (SELECT COUNT(*) FROM proformas) = 0 THEN
    ...
    VALUES (
      v_proj1_id, 'PRO-001',
      'Estructura Edificio Central UCB',
    ...
    INSERT INTO partidas_proforma (proforma_id, apu_id, cantidad, created_at) VALUES
      (v_proforma_id, v_apu2_id,  45.0, NOW()-INTERVAL '30 days'),
      (v_proforma_id, v_apu3_id, 120.0, NOW()-INTERVAL '30 days');

    -- Requerimientos: APU-002 × 45m³ + APU-003 × 120m²
    -- Cemento:  45 × 6.50 × 1.05 = 307.13
    -- Arena:    45 × 0.55 × 1.03 =  25.49
    -- Grava:    45 × 0.75 × 1.03 =  34.76
    -- Ladrillo: 120 × 55 × 1.05  = 6930
    INSERT INTO requerimientos (proforma_id, material_id, cantidad, created_at)
    VALUES (v_proforma_id, v_mat1_id, 307.13, NOW()-INTERVAL '29 days')
    RETURNING id INTO v_req1_id;

    INSERT INTO requerimientos (proforma_id, material_id, cantidad, created_at)
    VALUES (v_proforma_id, v_mat2_id, 25.49, NOW()-INTERVAL '29 days')
    RETURNING id INTO v_req2_id;

    INSERT INTO requerimientos (proforma_id, material_id, cantidad, created_at)
    VALUES (v_proforma_id, v_mat3_id, 34.76, NOW()-INTERVAL '29 days')
    RETURNING id INTO v_req3_id;

    INSERT INTO requerimientos (proforma_id, material_id, cantidad, created_at)
    VALUES (v_proforma_id, v_mat7_id, 6930, NOW()-INTERVAL '29 days')
    RETURNING id INTO v_req4_id;

    -- Segunda proforma (borrador) para PROY-002
    INSERT INTO proformas
      (proyecto_id, codigo, nombre, descripcion,
       fecha_elaboracion, elaborado_por, estado, created_at, updated_at)
    VALUES (
      v_proj2_id, 'PRO-002',
      'Superestructura Puente Río Choqueyapu',
  ...
  ELSE
    -- Si ya hay proformas, recuperar IDs para las OC
  ...
  IF (SELECT COUNT(*) FROM ordenes_compra) = 0 AND v_req1_id IS NOT NULL THEN
    ...
    -- OC-001: PENDIENTE — Cemento (stock bajo mínimo)
    ...
    VALUES (v_req1_id, v_mat1_id, v_prov1_id, 'SOBOCE S.A.',
            ...
            NOW()-INTERVAL '5 days',
            'Reposición urgente — stock cayó por debajo del mínimo operativo (50 bolsas)');

    -- OC-002: APROBADA — Grava (requerimiento de proforma)
    INSERT INTO ordenes_compra
      (requerimiento_id, material_id, proveedor_id, nombre_proveedor,
       cantidad, precio_unitario, costo_estimado,
    ...
    VALUES (v_req3_id, v_mat3_id, v_prov1_id, 'SOBOCE S.A.',
            ...
            'Aprobada por Gerencia — entrega programada para la semana siguiente');

    -- OC-003: RECIBIDA — Ladrillo (histórico completado)
    INSERT INTO ordenes_compra
      (requerimiento_id, material_id, proveedor_id, nombre_proveedor,
       cantidad, precio_unitario, costo_estimado,
       estado, generada_por, aprobada_por,
